...
public class ECKey implements EncryptableItem, Serializable {
     ...
     * @throws org.spreadcoinj.core.ECKey.MissingPrivateKeyException if the private key is missing or encrypted.
     */
    public byte[] toASN1() {
        try {
     ...
     * usually encoded using ASN.1 format, so you want {@link org.spreadcoinj.core.ECKey.ECDSASignature#toASN1()}
    ...
    /**
     * Signs the given hash and returns the R and S components as BigIntegers. In the Bitcoin protocol, they are
     * usually encoded using DER format, so you want {@link org.spreadcoinj.core.ECKey.ECDSASignature#encodeToDER()}
    ...
    public static ECKey signedToKey(Sha256Hash hash, byte[] signatureEncoded, boolean compressed) throws SignatureException {
        // Parse the signature bytes into r/s and the selector value.
        if (signatureEncoded.length < 65)
            throw new SignatureException("Signature truncated, expected 65 bytes and got " + signatureEncoded.length);
        int header = signatureEncoded[0] & 0xFF;
     ...
     * @throws org.spreadcoinj.core.ECKey.MissingPrivateKeyException if the private key bytes are missing/encrypted.
     */
     ...
     * the {@link org.spreadcoinj.core.DumpedPrivateKey#toString()} method to get the string.
     *
