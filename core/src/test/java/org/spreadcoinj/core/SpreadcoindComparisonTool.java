...
import org.spreadcoinj.store.H2FullPrunedBlockStore;
...
/**
 * A tool for comparing the blocks which are accepted/rejected by bitcoind/spreadcoinj
 * It is designed to run as a testnet-in-a-box network between a single bitcoind node and spreadcoinj
 * It is not an automated unit-test because it requires a bit more set-up...read comments below
 */
public class SpreadcoindComparisonTool {
    private static final Logger log = LoggerFactory.getLogger(SpreadcoindComparisonTool.class);
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("USAGE: spreadcoinjBlockStoreLocation runLargeReorgs(1/0) [port=18444]");
        ...
        for (Rule rule : blockList.list) {
            if (rule instanceof BlockAndValidity) {
                ...
                if (!chain.getChainHead().getHeader().getHash().equals(bitcoindChainHead)) {
                    differingBlocks++;
                    log.error("bitcoind and spreadcoinj acceptance differs on block \"" + block.ruleName + "\"");
                }
        ...
        log.info("Done testing.\n" +
                "Blocks which were not handled the same between bitcoind/spreadcoinj: " + differingBlocks + "\n" +
